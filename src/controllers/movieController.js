...
export async function getAllMoviesController(req, res) {
    try {
        ...
        const genre = req.query.genre; // TODO (ejercicio 3): leer también genre de los query params
        const pelis = await getAllMovies({ page, limit, genre });
        res.json(pelis);
    } catch (error) {
...
export async function getMovieController(req, res) {
    try {
        const peli = await getMovieByID(req.params.id);
        if(!peli){
